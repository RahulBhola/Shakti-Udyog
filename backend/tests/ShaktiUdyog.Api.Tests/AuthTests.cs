...
public class AuthApiFactory : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        // Login integration tests exercise the classic single-step flow; the
        // OTP step is covered by a dedicated test class with OTP enabled.
        builder.UseSetting("Auth:OtpEnabled", "false");
    }
}
...
/// <summary>
/// Email-OTP login integration tests. Unlike the auth suite (OTP disabled),
/// these run with the OTP step enabled against the local dev database. In
/// Development the login challenge echoes the code (<c>DebugOtp</c>), which is
/// how the flow completes here without a mail server. Skipped implicitly when
/// the dev admin seed password is absent.
/// </summary>
public class OtpApiFactory : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        builder.UseEnvironment("Development");
        builder.UseSetting("RateLimits:PublicPerMinute", "1000");
        builder.UseSetting("Auth:OtpEnabled", "true");
        builder.UseSetting("Auth:DebugOtpInDevelopment", "true");
    }
}

public class OtpEndpointTests(OtpApiFactory factory) : IClassFixture<OtpApiFactory>
{
    private readonly HttpClient _client = factory.CreateClient();

    private sealed record LoginChallenge(bool RequiresOtp, Guid ChallengeId, string? DebugOtp);
    private sealed record AuthResponse(string AccessToken, DateTimeOffset AccessTokenExpiresAtUtc, string RefreshToken, string TokenType);

    private async Task<(string? Password, LoginChallenge? Challenge)> StartOtpLoginAsync()
    {
        var cfg = factory.Services.GetService(typeof(Microsoft.Extensions.Configuration.IConfiguration))
            as Microsoft.Extensions.Configuration.IConfiguration;
        var password = Environment.GetEnvironmentVariable("DevAdmin__Password") ?? cfg?["DevAdmin:Password"];
        ...
        return (password, await response.Content.ReadFromJsonAsync<LoginChallenge>());
    }

    [Fact]
    public async Task Correct_password_returns_otp_challenge_not_tokens()
    {
        var (password, challenge) = await StartOtpLoginAsync();
        if (password is null || challenge is null)
        {
            return; // Dev admin not seeded in this environment.
        }

        Assert.True(challenge.RequiresOtp);
        Assert.NotEqual(Guid.Empty, challenge.ChallengeId);
        Assert.NotNull(challenge.DebugOtp); // Development echo lets the flow complete without a mail server.
    }

    [Fact]
    public async Task Verify_otp_issues_tokens_code_is_single_use_and_resend_works()
    {
        var (password, challenge) = await StartOtpLoginAsync();
        if (password is null || challenge is null)
        {
        ...
        var wrong = await _client.PostAsJsonAsync("/api/v1/auth/verify-otp",
            new { challengeId = challenge.ChallengeId, code = "000000" });
        Assert.Equal(HttpStatusCode.BadRequest, wrong.StatusCode);

        var good = await _client.PostAsJsonAsync("/api/v1/auth/verify-otp",
            new { challengeId = challenge.ChallengeId, code = challenge.DebugOtp });
        Assert.Equal(HttpStatusCode.OK, good.StatusCode);
        var tokens = await good.Content.ReadFromJsonAsync<AuthResponse>();
        Assert.NotNull(tokens);
        Assert.False(string.IsNullOrEmpty(tokens!.AccessToken));

        // Reusing the consumed code must fail (single-use).
        var reuse = await _client.PostAsJsonAsync("/api/v1/auth/verify-otp",
            new { challengeId = challenge.ChallengeId, code = challenge.DebugOtp });
        Assert.Equal(HttpStatusCode.BadRequest, reuse.StatusCode);

        // A fresh login allows a resend of a still-usable challenge.
        var (_, fresh) = await StartOtpLoginAsync();
        if (fresh is null)
        {
            return;
        }
        var resend = await _client.PostAsJsonAsync("/api/v1/auth/resend-otp",
            new { challengeId = fresh.ChallengeId });
