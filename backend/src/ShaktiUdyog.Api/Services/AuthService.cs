...
{
    /// <summary>
    /// Validates credentials and either issues tokens (OTP disabled) or mints an
    /// email OTP challenge (OTP enabled). Returns null on invalid credentials so
    /// the caller can reply uniformly.
    /// </summary>
    Task<LoginResult?> LoginAsync(LoginRequest request, string? ipAddress, string? userAgent);
    Task<AuthResponse?> VerifyOtpAsync(VerifyOtpRequest request, string? ipAddress, string? userAgent);
    Task<bool> ResendOtpAsync(ResendOtpRequest request, string? ipAddress, string? userAgent);
    ...
    ILogger<AuthService> logger) : IAuthService
{
    public async Task<LoginResult?> LoginAsync(LoginRequest request, string? ipAddress, string? userAgent)
    {
        ...
        {
            var (challengeId, code) = await emailOtpService.CreateAsync(user, ipAddress);
            await emailSender.SendAsync(
                user.Email!,
                "Shakti Udyog login code",
                $"Your Shakti Udyog verification code is {code}. It expires in 5 minutes. "
                + "If you did not request this, you can safely ignore this email.");

            await audit.WriteAsync("auth.login.otp_sent", user.Id, "User", user.Id.ToString(), ipAddress, userAgent);
            var debugOtp = env.IsDevelopment() && authOptions.Value.DebugOtpInDevelopment ? code : null;
            return new LoginOtpChallenge(challengeId, debugOtp);
        }
        ...
        return new LoginTokens(new AuthResponse(access.Token, access.ExpiresAtUtc, refresh.RawToken));
    }

    public async Task<AuthResponse?> VerifyOtpAsync(VerifyOtpRequest request, string? ipAddress, string? userAgent)
    {
        var user = await emailOtpService.VerifyAsync(request.ChallengeId, request.Code);
        if (user is null)
        {
            await audit.WriteAsync("auth.otp.failed", null, null, null, ipAddress, userAgent);
            return null;
        }
    ...
    public async Task<bool> ResendOtpAsync(ResendOtpRequest request, string? ipAddress, string? userAgent)
    {
        var result = await emailOtpService.ResendAsync(request.ChallengeId, ipAddress);
        if (result is null)
        {
            await audit.WriteAsync("auth.otp.resend.failed", null, null, null, ipAddress, userAgent);
            return false;
        }

        var (_, code, user) = result.Value;
        await emailSender.SendAsync(
            user.Email!,
            "Shakti Udyog login code",
            $"Your Shakti Udyog verification code is {code}. It expires in 5 minutes. "
        ...
        await audit.WriteAsync("auth.otp.resend", user.Id, "User", user.Id.ToString(), ipAddress, userAgent);
        return true;
    }
