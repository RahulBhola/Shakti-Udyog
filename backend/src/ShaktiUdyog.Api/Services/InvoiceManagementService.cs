...
public class InvoiceManagementService(AppDbContext db, IAuditWriter audit, IPortalPush push) : IInvoiceManagementService
{
    public async Task<PagedResult<InvoiceListItemDto>> GetInvoicesAsync(int page, int pageSize, string? status, string? search)
    {
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            query = query.Where(i =>
                ...
                (i.Order != null && i.Order.OrderNumber.Contains(term)) ||
    ...
    public async Task<InvoiceDetailDto?> GetInvoiceAsync(Guid id) => await db.Invoices.Where(i => i.Id == id)
        .Include(i => i.Items).Select(i => new InvoiceDetailDto(i.Id, i.Order!.Id, i.InvoiceNumber, i.Order!.OrderNumber, i.IssueDateUtc, i.DueDateUtc, i.Subtotal, i.Tax, i.Total, i.AmountPaid, i.BalanceDue, i.Currency, i.Status, i.DocumentId, i.Company != null ? i.Company.Name : null,
            ...
            db.Payments.Where(p => p.InvoiceId == i.Id).Select(p => new PaymentDto(p.Id, p.PaymentReference, p.Method, p.Amount, p.PaymentDateUtc, p.Status, p.CreatedAtUtc, p.ProofDocumentId, p.VerificationNote)).ToList(),
    ...
    public async Task<bool> RecordPaymentAsync(Guid invoiceId, RecordPaymentRequest request, Guid userId, string? ip)
    {
        ...
        invoice.Status = invoice.BalanceDue <= 0 ? InvoiceStatuses.FullyPaid : InvoiceStatuses.PartiallyPaid;
    ...
    {
        var p = await db.Payments.Include(p => p.Invoice).SingleOrDefaultAsync(x => x.Id == paymentId);
        if (p is null) return false;
        p.Status = PaymentStatuses.Verified;
        p.VerifiedByUserId = userId;
        p.VerifiedAtUtc = DateTimeOffset.UtcNow;

        // Recompute the invoice from VERIFIED payments only; admin verification
        // is what moves the bill to "Fully Paid" / "Partially Paid". Count the
        // current payment explicitly (its Status change isn't saved to the DB yet).
        var invoice = p.Invoice;
        var previousStatus = invoice.Status;
        var otherVerifiedTotal = await db.Payments
            .Where(x => x.InvoiceId == invoice.Id && x.Status == PaymentStatuses.Verified && x.Id != paymentId)
            .SumAsync(x => x.Amount);
        var verifiedTotal = otherVerifiedTotal + p.Amount;
        invoice.AmountPaid = verifiedTotal;
        invoice.BalanceDue = invoice.Total - verifiedTotal;
        invoice.Status = invoice.BalanceDue <= 0 ? InvoiceStatuses.FullyPaid : InvoiceStatuses.PartiallyPaid;
        ...
        if (invoice.Status != previousStatus)
        {
            db.InvoiceStatusHistories.Add(new InvoiceStatusHistory
            {
                ...
                ChangedByRole = "Admin",
                Note = $"Payment verified: {p.PaymentReference}",
            });
        }

        await AddPaymentNotificationsAsync(
            invoice.CompanyId, p.SubmittedByUserId,
            "Payment verified",
            "Your payment has been verified successfully.",
            $"/customer/invoices/{invoice.Id}");

        await db.SaveChangesAsync();
        await push.PaymentVerifiedAsync(new PaymentVerifiedPayload(p.Id, invoice.Id, invoice.InvoiceNumber, p.Amount));
    ...
    public async Task<bool> RejectPaymentAsync(Guid paymentId, string reason, Guid userId, string? ip)
    {
        ...
        if (invoice is not null)
        {
            await AddPaymentNotificationsAsync(
                invoice.CompanyId, p.SubmittedByUserId,
                "Payment rejected",
                $"Your payment of {p.Amount:0.00} for invoice {invoice.InvoiceNumber} was rejected. Reason: {reason}. Please upload a new payment proof.",
                $"/customer/invoices/{invoice.Id}");
        }
    ...
    /// <summary>
    /// Creates in-app notifications for a payment outcome, addressed to the
    /// payment submitter and every user linked to the invoice's company.
    /// </summary>
    private async Task AddPaymentNotificationsAsync(
        Guid companyId, Guid? submitterId, string title, string body, string linkPath)
    {
        var userIds = new List<Guid>();
        if (submitterId is Guid s) userIds.Add(s);
        userIds.AddRange(await db.UserCompanies
        ...
        foreach (var uid in userIds.Distinct())
        {
            ...
            await push.NotificationCreatedAsync(uid, new NotificationCreatedPayload(
                notification.Id, notification.Title, notification.Body, notification.LinkPath, notification.CreatedAtUtc));
        }
    }
