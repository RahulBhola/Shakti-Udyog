...
public record CreateShipmentRequest(string? Transporter, string? TrackingNumber, string? VehicleNumber, string? PhoneNumber, DateTimeOffset? DispatchDateUtc, DateTimeOffset? EstimatedArrivalUtc);
    ...
    IAuditWriter audit) : IOrderUpdaterService
{
    ...
    public async Task<OrderDetailDto?> GetOrderAsync(Guid id)
    {
        ...
        var latestInvoice = await db.Invoices
            ...
            .Select(i => new OrderCommercialDto(i.InvoiceNumber, i.IssueDateUtc, i.DueDateUtc, i.Total, i.AmountPaid, i.BalanceDue, i.Status, null, (Guid?)i.Id))
            .FirstOrDefaultAsync();
    ...
    public async Task<bool?> CreateShipmentAsync(Guid id, CreateShipmentRequest request, Guid userId, string? ip)
    {
        ...
        db.Shipments.Add(new Shipment { Id = Guid.NewGuid(), OrderId = id, Transporter = request.Transporter, TrackingNumber = request.TrackingNumber, VehicleNumber = request.VehicleNumber, PhoneNumber = request.PhoneNumber, DispatchDateUtc = request.DispatchDateUtc, EstimatedArrivalUtc = request.EstimatedArrivalUtc });
        await db.SaveChangesAsync();
    ...
    public async Task<bool?> UpdateShipmentAsync(Guid orderId, Guid shipmentId, CreateShipmentRequest request, Guid userId, string? ip)
    {
        ...
        await db.SaveChangesAsync();
        await audit.WriteAsync("updater.order.shipment_updated", userId, "Shipment", shipmentId.ToString(), ip);
        return true;
    }
    ...
    public async Task<bool?> DeleteShipmentAsync(Guid orderId, Guid shipmentId, Guid userId, string? ip)
    {
        ...
        await db.SaveChangesAsync();
        await audit.WriteAsync("updater.order.shipment_deleted", userId, "Shipment", shipmentId.ToString(), ip);
        return true;
    }
