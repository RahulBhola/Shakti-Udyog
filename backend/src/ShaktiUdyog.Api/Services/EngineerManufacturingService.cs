    ...
    IAuditWriter audit) : IEngineerManufacturingService
{
    ...
    {
        var query = db.Orders
            .Where(o => o.AssignedEngineerId != null && o.Status != OrderStatuses.Cancelled);

        if (!isAdmin)
            query = query.Where(o => o.AssignedEngineerId == userId);
        ...
        return await query
            ...
            .Select(o => new EngineerOrderDto(
                ...
                o.OrderNumber,
                o.Company.Name,
                o.Quotation != null ? o.Quotation.Rfq.ProductType : null,
    ...
    public async Task<bool?> MoveStageAsync(Guid orderId, Guid userId, bool isAdmin, string targetStage, string? ip)
    {
        ...
        if (!isAdmin && order.AssignedEngineerId != userId) return null;
