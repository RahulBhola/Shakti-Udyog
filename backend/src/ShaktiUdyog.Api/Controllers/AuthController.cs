...
public class AuthController(IAuthService authService) : ControllerBase
{
    ...
    [ProducesResponseType<OtpChallengeResponse>(StatusCodes.Status200OK)]
    ...
    public async Task<IActionResult> Login(LoginRequest request)
    {
        ...
        switch (result)
        {
            ...
            case LoginOtpChallenge challenge:
                return Ok(new OtpChallengeResponse(true, challenge.ChallengeId, challenge.DebugOtp));
            default:
                var tokens = ((LoginTokens)result).Response;
    ...
    {
        var result = await authService.VerifyOtpAsync(request, ClientIp, UserAgent);
        if (result is null)
        {
            return BadRequest(new MessageResponse("Invalid or expired verification code."));
        }
    ...
    {
        var ok = await authService.ResendOtpAsync(request, ClientIp, UserAgent);
        return ok
            ? Ok(new MessageResponse("A new verification code has been sent."))
            : BadRequest(new MessageResponse("The verification session is invalid or expired."));
    }
