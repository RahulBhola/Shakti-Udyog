...
public class OrderUpdaterController(IOrderUpdaterService service) : ControllerBase
{
    ...
    [HttpPatch("orders/{orderId:guid}/shipments/{shipmentId:guid}")]
    public async Task<IActionResult> UpdateShipment(Guid orderId, Guid shipmentId, CreateShipmentRequest request)
    {
        var result = await service.UpdateShipmentAsync(orderId, shipmentId, request, UserId, ClientIp);
    ...
    {
        var result = await service.DeleteShipmentAsync(orderId, shipmentId, UserId, ClientIp);
