...
public class AdminController(IAdminService adminService, AppDbContext db, UserManager<ApplicationUser> userManager) : ControllerBase
{
    ...
    // ---- Engineer assignment & workload -------------------------------------

    [HttpGet("engineers")]
    [ProducesResponseType<IReadOnlyList<EngineerWorkloadDto>>(StatusCodes.Status200OK)]
    public async Task<IActionResult> GetEngineers()
    {
        return Ok(await adminService.GetEngineersWithWorkloadAsync());
    }

    [HttpPost("rfqs/{id:guid}/assign")]
    [ProducesResponseType<MessageResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> AssignRfq(Guid id, RfqAssignmentRequest request)
    {
        var result = await adminService.AssignEngineerAsync(id, request.AssignedToUserId, UserId, ClientIp);
        return result switch
        {
            null => NotFound(new MessageResponse("RFQ not found.")),
            false => BadRequest(new MessageResponse("Target user is not an engineer.")),
    ...
    /// <summary>Assign (or clear) the engineer responsible for an order's manufacturing.</summary>
    [HttpPatch("orders/{orderId:guid}/engineer")]
    public async Task<IActionResult> AssignOrderEngineer(Guid orderId, [FromBody] AssignOrderEngineerRequest request)
    {
        var result = await adminService.AssignOrderEngineerAsync(orderId, request.EngineerId, UserId, ClientIp);
        return result switch
        {
            null => NotFound(),
            _ => Ok(new { message = request.EngineerId is null ? "Engineer unassigned." : "Engineer assigned." }),
        };
    }
...
public record AssignOrderEngineerRequest(Guid? EngineerId);
