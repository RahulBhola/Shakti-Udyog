    ...
    IFileStorageService storage) : ControllerBase
{
    ...
    /// <summary>Admins have full RFQ visibility; engineers only see assigned RFQs.</summary>
    ...
    [HttpGet("me")]
    [ProducesResponseType<UpdaterMeDto>(StatusCodes.Status200OK)]
    public async Task<IActionResult> GetMe() => Ok(await updaterService.GetMeAsync(UserId));
    ...
    {
        return Ok(await updaterService.GetRfqsAsync(page, pageSize, search, status, UserId, !IsAdmin));
    }
    ...
    {
        var rfq = await updaterService.GetRfqAsync(id, UserId, !IsAdmin);
    ...
    {
        var comment = await updaterService.AddRfqCommentAsync(id, request, UserId, UserRole, ClientIp, !IsAdmin);
    ...
    public async Task<IActionResult> DownloadRfqFile(Guid id, Guid fileId)
    {
        if (!IsAdmin)
        {
            // Engineers may only download files of RFQs assigned to them.
            var assigned = await db.RfqAssignments
                .AnyAsync(a => a.RfqId == id && a.IsActive && a.AssignedToUserId == UserId);
