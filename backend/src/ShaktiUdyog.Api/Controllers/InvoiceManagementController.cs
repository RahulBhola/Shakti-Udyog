    ...
    IAuditWriter audit) : ControllerBase
{
    ...
    public async Task<IActionResult> GetPendingPayments()
    {
        var payments = await db.Payments
            ...
            .Select(p => new
            {
                ...
                p.ProofDocumentId, p.VerificationNote,
    ...
    /// <summary>
    /// Streams the customer-uploaded payment proof document for a payment so an
    /// admin can review it before verifying/rejecting. File is served from
    /// protected storage; access is logged.
    /// </summary>
    [HttpGet("admin/payments/{paymentId:guid}/proof")]
    [Authorize(Policy = AuthPolicies.AdminOnly)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> GetPaymentProof(Guid paymentId)
    {
        var documentId = await db.Payments
            .Where(p => p.Id == paymentId)
            .Select(p => p.ProofDocumentId)
            .SingleOrDefaultAsync();
        if (documentId is null)
        {
            return NotFound(new { message = "No payment proof has been uploaded for this payment." });
        }

        var doc = await documentService.GetAsync(documentId.Value);
        if (doc is null) return NotFound();

        var file = await documentService.DownloadAsync(doc.Id);
        if (file is null) return NotFound();

        await audit.WriteAsync("admin.payment.proof_downloaded", UserId, "Payment", paymentId.ToString(), ClientIp);
