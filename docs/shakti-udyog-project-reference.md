...
## 11b. Engineer Manufacturing Board, Payments & Realtime

### Engineer Manufacturing Board (order-level Kanban)

- Route: `GET /api/v1/engineer/orders` — orders on the board; an engineer sees **only** their assigned orders (`AssignedEngineerId == user`), admins see all. `[Authorize(EngineerOnly)]`.
- `PATCH /api/v1/engineer/orders/{id}/stage` body `{ "stage": "..." }` — advances an order **one column forward** (`pattern_development → production → quality_check → packed → ready_to_dispatch`). Backend rejects skip/backward with `409` via `ManufacturingStages.IsValidForwardTransition` (`backend/src/ShaktiUdyog.Domain/Constants/ManufacturingStages.cs`). On success it: sets `Order.ManufacturingStage`/`StageUpdatedAt` (and keeps `Order.Status` in sync), appends an `OrderMilestone`, writes a `Notification` (Type `Order`) for the company users, broadcasts `stageChanged`, and audits.
- Admin assignment: `PATCH /api/v1/admin/orders/{id}/engineer` body `{ "engineerId": "..." }` (null clears). Orders are auto-assigned from the RFQ's active engineer on quotation acceptance.
- Frontend: `frontend/src/portal/pages/EngineerBoardPage.tsx` at `/admin/manufacturing` (nav label "My Manufacturing Board" for engineers), client `frontend/src/api/engineerApi.ts`.

### Realtime (SignalR)

- Hub `PortalHub` at `/hubs/portal` (`backend/src/ShaktiUdyog.Api/Hubs/PortalHub.cs`), JWT auth (token accepted via `access_token` query on the hub path).
- Connection groups are derived from claims on connect: `user:{userId}` (private), `admins`, `engineers`.
- Events: `stageChanged` (engineers + admins), `notificationCreated` (a specific user), `paymentVerified` (admins).
- Frontend singleton client `frontend/src/realtime/signalR.ts` (`@microsoft/signalr`). The customer notification bell subscribes to `notificationCreated` so the unread badge bumps live; admin order detail, customer order detail, and the engineer board subscribe to `stageChanged` to refresh timelines/columns instantly.

### Customer Pay Now

- `QuotationDetailDto` now includes `invoiceId`, `paymentStatus` (`Unpaid`/`Partially Paid`/`Paid`), and `paymentProofStatus`. On an accepted quotation with an unpaid invoice, `frontend/src/portal/pages/QuotationsPage.tsx` shows a **Pay Now** button that links to `/customer/invoices/{id}` (bank details + "upload payment proof" → `Payment.Status = Pending Verification`; the invoice is NOT marked paid).
- `Payment` gained `VerifiedByUserId` + `VerifiedAtUtc` (set by `InvoiceManagementService.VerifyPaymentAsync`, which also emits `paymentVerified` and the notification "Your payment has been verified successfully.").

### Admin Payment Proof Review

- In Finance → Invoices, each Pending Payment Approval row now has a **View Payment Proof** button beside **Verify → Fully Paid**. The modal shows the uploaded image/PDF (fetched via the protected `GET /api/v1/admin/payments/{id}/proof`), transaction reference, amount, payment date, customer, and invoice number.
