...
import { ArrowRight, Sparkles, Heart, ShieldCheck, Weight, Layers } from 'lucide-react';
    ...
    <section className={`relative py-16 sm:py-24 overflow-hidden transition-colors duration-300 ${
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 mb-10 sm:mb-14">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-mono font-semibold tracking-wider uppercase text-orange-500 bg-orange-500/10 border border-orange-500/20">
          ...
          <Link
            to="/products"
            className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full text-xs sm:text-sm font-semibold tracking-wide text-white bg-gradient-to-r from-orange-600 to-amber-500 hover:from-orange-500 hover:to-amber-400 shadow-[0_4px_20px_rgba(255,109,0,0.3)] transition-all transform hover:-translate-y-0.5"
          >
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
          <div className={`relative w-full max-w-xl p-6 sm:p-8 rounded-3xl border shadow-2xl transition-all ${
            isLight ? 'bg-white border-neutral-200 text-neutral-900' : 'bg-[#0f1015] border-white/10 text-white'
          }`}>
            <button
              onClick={() => setSelectedProduct(null)}
              className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center bg-neutral-500/20 hover:bg-neutral-500/40 text-neutral-400 hover:text-white transition-colors"
            >
              ✕
            </button>

            <div className="flex flex-col sm:flex-row items-center gap-6">
              <div className="w-48 h-48 sm:w-56 sm:h-56 rounded-2xl bg-[#08080a] flex items-center justify-center p-4 border border-white/5 shadow-inner shrink-0">
                <img
              ...
              <div className="space-y-3 w-full">
                <div className="inline-block px-2.5 py-1 rounded-md text-[11px] font-mono uppercase bg-orange-500/15 text-orange-400 font-bold border border-orange-500/30">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs font-mono pt-2">
                  <div className="p-2 rounded-lg bg-white/5 border border-white/5">
                    ...
                    <span className="font-semibold text-orange-400">{selectedProduct.grade}</span>
                  </div>
                  <div className="p-2 rounded-lg bg-white/5 border border-white/5">
                    <span className="text-neutral-400 block">Piece Weight</span>
                ...
                <div className="pt-3 flex gap-3">
                  <Link
                    to="/request-a-quote"
                    className="w-full text-center py-2.5 rounded-full text-xs font-semibold tracking-wide text-white bg-orange-500 hover:bg-orange-600 transition-all shadow-[0_0_15px_rgba(255,109,0,0.3)]"
                  >
...
const ProductGalleryCard: React.FC<CardProps> = ({ item, isLight, onSelect }) => {
  return (
    <div
      onClick={onSelect}
      className={`group relative w-64 sm:w-72 h-80 rounded-3xl p-4 sm:p-5 flex flex-col justify-between cursor-pointer border transition-all duration-300 transform hover:-translate-y-1.5 hover:shadow-2xl ${
        isLight
    ...
    >
      ...
      <div className="flex items-center justify-between text-[11px]">
        <span className="px-2.5 py-0.5 rounded-full font-mono font-medium text-orange-500 bg-orange-500/10 border border-orange-500/20">
          {item.category}
        </span>
      ...
      <div className="relative w-full h-36 my-2 flex items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-b from-transparent to-black/20">
        <img
          src={item.image}
          alt={item.title}
          loading="lazy"
          className="max-h-32 max-w-[85%] object-contain transition-transform duration-500 group-hover:scale-110 drop-shadow-[0_8px_16px_rgba(0,0,0,0.6)]"
        />
      </div>

      {/* Card Info Footer */}
      <div className="space-y-1.5 pt-2 border-t border-white/[0.06] dark:border-white/[0.06]">
        <h4 className={`text-sm font-bold tracking-tight line-clamp-1 group-hover:text-orange-500 transition-colors ${
          isLight ? 'text-neutral-900' : 'text-white'
        }`}>
          {item.title}
        </h4>
