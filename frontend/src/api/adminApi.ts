import { apiGet, apiPatch, apiPost, apiPut, apiDelete, apiUpload, ApiError } from "./client";
...
/** An Engineer-role user with their current active-RFQ workload (for assignment). */
...
export const adminApi = {
  ...
  engineers: () => apiGet<EngineerWorkload[]>(`${base}/engineers`),
  assignRfq: (id: string, assignedToUserId: string) =>
    apiPost<{ message: string }>(`${base}/rfqs/${id}/assign`, { assignedToUserId }),
  ...
  assignOrderEngineer: (orderId: string, engineerId: string) =>
    apiPatch<{ message: string }>(`${base}/orders/${orderId}/engineer`, { engineerId }),
  ...
  pendingPayments: () =>
    apiGet<{
      ...
      proofDocumentId: string | null; verificationNote: string | null;
  ...
  paymentProofDownloadUrl: (paymentId: string) => `${base}/payments/${paymentId}/proof`,
  /** Fetches the uploaded payment proof (protected) and returns an object URL for inline preview. */
  paymentProofPreview: async (paymentId: string): Promise<{ url: string; contentType: string; name: string } | null> => {
    const token = tokenStorage.getAccessToken();
    const res = await fetch(`${config.apiBaseUrl}${base}/payments/${paymentId}/proof`, {
    ...
    const name = match?.[1] ? decodeURIComponent(match[1]) : "payment-proof";
    return { url: URL.createObjectURL(await res.blob()), contentType, name };
  },
