import { apiGet, apiPost, apiPatch, apiDelete, apiPut, apiUpload } from './client';
...
export interface EngineerEnquiryDetail {
  ...
  comments: { id: string | null; authorUserId: string; authorRole: string; isCustomerVisible: boolean; message: string; createdAtUtc: string }[];
  assignedToUserId: string | null;
  priority: string;
...
export const engineerApi = {
  // ---- Dashboard ----------------------------------------------------------
  dashboard: () => apiGet<EngineerDashboard>(`${base}/engineer-board`),
  ...
  createQuotation: (payload: Record<string, unknown>) => apiPost<{ id: string }>(`${base}/quotations`, payload),
  updateQuotation: (id: string, payload: Record<string, unknown>) => apiPut<{ message: string }>(`${base}/quotations/${id}`, payload),
  submitQuotation: (id: string) => apiPost<{ message: string }>(`${base}/quotations/${id}/submit`),
  ...
  getOrderComments: (id: string) => apiGet<{ authorRole: string; authorName: string | null; message: string; createdAtUtc: string }[]>(
    `${base}/orders/${id}/comments`),
  ...
  createEngineer: (request: { fullName: string; password: string }) =>
    apiPost<{ email: string; userId: string; fullName: string }>(`${base}/engineers`, request),
