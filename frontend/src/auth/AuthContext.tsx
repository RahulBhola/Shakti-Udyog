...
import { authService, type AuthUser, type LoginResult } from "./authService";
...
interface AuthContextValue {
  ...
  isLoading: boolean;
  /** Returns whether credentials passed and whether an OTP step is required. */
  login: (email: string, password: string) => Promise<LoginResult>;
  /** Completes the email-OTP step; on success the session is established. */
  verifyOtp: (challengeId: string, code: string) => Promise<boolean>;
  resendOtp: (challengeId: string) => Promise<boolean>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const verifyOtp = useCallback(async (challengeId: string, code: string) => {
    const ok = await authService.verifyOtp(challengeId, code);
    if (ok) {
      setUser(await authService.me());
    }
    return ok;
  ...
  const resendOtp = useCallback(
    (challengeId: string) => authService.resendOtp(challengeId),
    [],
  );
  ...
  const value = useMemo(
    () => ({ user, isLoading, login, verifyOtp, resendOtp, loginWithProvider, logout }),
    [user, isLoading, login, verifyOtp, resendOtp, loginWithProvider, logout],
  );
