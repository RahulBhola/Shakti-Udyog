...
/*  CSV export                                                        */
function exportToCsv(items: UpdaterRfqListItem[], includeCustomer = true) {
  const headers = [
    "RFQ No.", ...(includeCustomer ? ["Customer"] : []),
    "Product", "Quantity", "Status", "Date", "Files", "Assigned",
  ];
  const rows = items.map((r) => [
    rfqNo(r.id),
    ...(includeCustomer ? [r.companyName ?? "Unknown"] : []),
    r.productType,
    r.quantity,
    r.status,
    formatDate(r.createdAtUtc),
    String(r.fileCount),
    r.assignedToName ?? (r.assignedToUserId ? "Yes" : "No"),
  ]);
  ...
  const renderRow = (r: UpdaterRfqListItem) => {
    return (
      <tr key={r.id} onClick={() => openRfq(r)}>
        ...
        <td>
          {r.assignedToName ? (
            <div className="inv-customer__name" style={{ fontSize: 13 }}>{r.assignedToName}</div>
          ) : r.assignedToUserId ? (
  ...
  return (
    <div className="inv-page">
      ...
      <div className="inv-header">
        <div>
          <h1 className="inv-header__title">{isEngineer ? "My Assigned RFQs" : "RFQs"}</h1>
          <p className="inv-header__subtitle">
            {isEngineer ? "RFQs assigned to you by the administrator." : "Manage customer Requests for Quotation."}
          </p>
        </div>
        <div className="inv-header__actions">
          <button className="inv-btn" onClick={() => { if (data) exportToCsv(data.items, !isEngineer); }} title="Export visible RFQs to Excel">
