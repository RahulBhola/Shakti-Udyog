...
import {
  ...
  Phone, Mail, Download, Printer, Share2, Clock, MessageSquare, CreditCard,
} from "lucide-react";
...
export function QuotationDetailPage() {
  ...
  // Fetch the generated order's status once an order exists so the Pay Now CTA
  // is hidden if that order was cancelled (order cancel doesn't change the quotation).
  useEffect(() => {
    setOrderStatus(null);
    if (!quotation?.orderId) return;
    customerApi.order(quotation.orderId).then((o) => setOrderStatus(o.status)).catch(() => {});
  }, [quotation?.orderId]);
  ...
  function printQuotationDoc() {
    ...
    const fmtDate = (d: string | null) => d ? new Date(d).toLocaleDateString("en-IN", { year: "numeric", month: "long", day: "numeric" }) : "—";
...
async function respond() {
    ...
    try {
      ...
      // On acceptance the generic "we will follow up" toast is replaced by the
      // Pay Now button, which appears once the refreshed quotation reports the
      // generated (unpaid) invoice.
      setMessage(responding === "accept" ? null : result.message);
  ...
  // Payment CTA: null (hidden) | "pay" (show the prominent Pay Now) | "awaiting"
  // (proof already submitted, waiting on admin verification).
  const paymentCta: "pay" | "awaiting" | null = (() => {
    if (quotation.status === "Cancelled" || orderStatus === "cancelled") return null;
    const payable = (quotation.status === "Accepted" || quotation.status === "Converted") && !!quotation.invoiceId;
    if (!payable) return null;
    if (quotation.paymentStatus === "Paid" || quotation.paymentProofStatus === "Verified") return null;
    if (quotation.paymentProofStatus === "Pending Verification") return "awaiting";
    return "pay";
  })();
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {/* ── Payment CTA ── */}
      {paymentCta === "pay" && quotation.invoiceId && (
        <div className="rounded-[16px] border border-[var(--color-primary)]/30 bg-gradient-to-br from-[var(--color-primary)]/10 to-[var(--color-primary-hover)]/10 p-5 sm:p-6 shadow-sm relative overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-start gap-3 min-w-0">
              <span className="flex items-center justify-center w-10 h-10 rounded-[12px] bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-hover)] text-white shrink-0">
                <Banknote size={18} />
              </span>
              <div className="min-w-0">
                <div className="text-[15px] font-semibold text-[var(--text-primary)]">Payment Required</div>
                <div className="text-[13px] text-[var(--text-secondary)] mt-0.5">Complete your payment to confirm this order — you&apos;ll see the bank details and upload your proof on the invoice page.</div>
              </div>
            </div>
            <Link to={`/customer/invoices/${quotation.invoiceId}`}
              className="inline-flex items-center justify-center gap-2.5 h-12 px-6 rounded-[12px] bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-hover)] text-white text-[15px] font-bold shadow-lg hover:shadow-xl hover:brightness-110 ring-1 ring-white/20 ring-inset transition-all duration-200 no-underline hover:no-underline shrink-0 w-full sm:w-auto">
      ...
      {paymentCta === "awaiting" && (
        <div className="rounded-[16px] border border-amber-300/60 dark:border-amber-500/30 bg-amber-50/80 dark:bg-amber-500/10 p-5 flex items-start gap-3">
          <span className="flex items-center justify-center w-9 h-9 rounded-[10px] bg-amber-500/10 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5">
            <Clock size={18} />
          </span>
          <div>
            <div className="text-[14px] font-semibold text-[var(--text-primary)]">Payment Proof Submitted</div>
            <div className="text-[13px] text-[var(--text-secondary)] mt-0.5">Awaiting Verification — your payment is being reviewed by our team.</div>
          </div>
        </div>
      )}
