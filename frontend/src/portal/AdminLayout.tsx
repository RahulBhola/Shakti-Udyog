...
// Engineers work on RFQs assigned to them, the shared manufacturing board,
// and their own employee detail. The board backend is Engineer+Admin; admin-only
// actions (delete job, etc.) stay hidden for engineers.
const engineerSections: NavSection[] = [
  {
    label: null,
    items: [
      { label: "My Assigned RFQs", href: "/admin/rfqs" },
      { label: "My Manufacturing Board", href: "/admin/manufacturing" },
      { label: "Employee Detail", href: "/admin/employee" },
    ],
  },
];
...
export default function AdminLayout() {
  ...
  // Engineers get an RFQ-only portal; admins see the full admin shell.
  const isEngineer = user?.roles.includes("Engineer") === true && user?.roles.includes("Admin") !== true;
  const sections = isEngineer ? engineerSections : adminSections;
  const portalLabel = isEngineer ? "Engineer Portal" : "Admin Portal";
    ...
    <div className="admin-portal-layout">
      ...
      <div className="admin-portal-main">
        ...
        <header className="portal__topbar">
          <strong className="text-[15px] font-semibold text-[var(--text-primary)]">{portalLabel}</strong>
        ...
        {/* Mobile Nav */}
        <nav className="portal-mobile-nav" aria-label={portalLabel}>
